...
def _start(file_path,total_frames,include_score,threshold):
    ...
    while frame_no < int(total_frames):
        ...
        if(ret == False):
            if frame_no == 0:
                print("Scaling method can't read video")
            break
        ...
        try:
            ...
            for box in data[frame_no]['people']:
                if(box[4]==1):
                    ...
                    xl,yl,xr,yr,keypoint_score = keypoint_location(crop_img, keypoints, bbox[0], bbox[1], threshold)
                ...
                if include_score:
                    ...
                    if(math.isnan(y_out)):
                        ...
                        export_to_ndjson_with_scores(frame_no, int(box[0]), int(box[1]), int(box[2]), int(y_out+(0.0138*height)), x_out, y_out, box[5], keypoint_score)
                    ...
                    if(math.isnan(y_out)):
                        ...
                        export_to_ndjson(frame_no, int(box[0]), int(box[1]), int(box[2]), int(y_out+(0.0138*height)), x_out, y_out)
    ...
    try:
        generate_json(file_name,total_frames=frame_no)
    ...
    print("Scaling complete.... Processing time: "+"%02d" % (hr)+":"+"%02d" % (mins)+":"+"%02d" % (seconds))    
